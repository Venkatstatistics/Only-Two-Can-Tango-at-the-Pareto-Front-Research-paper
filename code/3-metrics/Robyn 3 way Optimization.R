...
remotes::install_github("febpeter/Robyn_updated_v2/R", force = TRUE)
library(Robyn)
...
robyn_directory <- "D:/Febin/Robyn Pareto Optimization R&D"
...
print(OutputModels)
library(dplyr)
results1 <- OutputModels$trial1$resultCollect$xDecompAgg %>% select(solID, nrmse, decomp.rssd, MAPE_train) %>% unique()
results2 <- OutputModels$trial2$resultCollect$xDecompAgg %>% select(solID, nrmse, decomp.rssd, MAPE_train) %>% unique()
results3 <- OutputModels$trial3$resultCollect$xDecompAgg %>% select(solID, nrmse, decomp.rssd, MAPE_train) %>% unique()
results4 <- OutputModels$trial4$resultCollect$xDecompAgg %>% select(solID, nrmse, decomp.rssd, MAPE_train) %>% unique()
results5 <- OutputModels$trial5$resultCollect$xDecompAgg %>% select(solID, nrmse, decomp.rssd, MAPE_train) %>% unique()

results_combined <- rbind(results1, results2, results3, results4, results5)
write.csv(results_combined, "objective_fn_values_3way_mape.csv")
