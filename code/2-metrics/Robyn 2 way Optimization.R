...
results1 <- OutputModels$trial1$resultCollect$xDecompAgg %>% select(solID, nrmse, decomp.rssd) %>% unique()
results2 <- OutputModels$trial2$resultCollect$xDecompAgg %>% select(solID, nrmse, decomp.rssd) %>% unique()
results3 <- OutputModels$trial3$resultCollect$xDecompAgg %>% select(solID, nrmse, decomp.rssd) %>% unique()
results4 <- OutputModels$trial4$resultCollect$xDecompAgg %>% select(solID, nrmse, decomp.rssd) %>% unique()
results5 <- OutputModels$trial5$resultCollect$xDecompAgg %>% select(solID, nrmse, decomp.rssd) %>% unique()

results_combined <- rbind(results1, results2, results3, results4, results5)
write.csv(results_combined, "objective_fn_values_2way.csv")
